...
>
> ⚠️ **BST(二叉搜索树)内容已独立成 binary_search_tree_notes.md**——两个武器、和二分的连线、700/701/98/230/235/530/501/450/669/776/270 全部在那份里。本文件专注普通二叉树(分治+遍历两大流派)。
...
```
两版都对:-1版更"炫技"简洁;tuple版更直白、更好迁移(BST的776也用这招,详见BST笔记)。
...
**★BST中序 = 从小到大有序!**——中序的这个特性是BST武器2的核心,详见 binary_search_tree_notes.md。
...
普通二叉树里226(翻转)就是这个模式(`root.left, root.right = right, left` 再 `return root`);BST的701/450/669/776是这个模式的深度应用,完整讲解见 binary_search_tree_notes.md 第七节。
